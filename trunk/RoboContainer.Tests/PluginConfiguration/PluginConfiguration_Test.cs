...
namespace RoboContainer.Tests.PluginConfiguration
{
	...
	public class PluginConfiguration_Test
	{
		...
		public void also_use_value_for_concrete_types()
		{
			...
			var container = new Container(c => c.ForPlugin<Foo1>().UseInstance(specialValue).UsePluggablesAutosearch(true));
				...
				c =>
					{
						...
						c.ForPlugin<IFoo>().UsePluggable<Foo1>("a", "b").UsePluggablesAutosearch(true);
					});
		...
		public void can_also_use_several_values()
		{
			...
			var container = new Container(c => c.ForPlugin<IFoo>().UseInstance(specialValue1).UseInstance(specialValue2).UsePluggablesAutosearch(true));
		...
		{
			var specialValue = new Foo1();
			var container = new Container(c => c.ForPlugin<IFoo>().UseInstance(specialValue).UsePluggablesAutosearch(true));
			IEnumerable<IFoo> foos = container.GetAll<IFoo>();
			CollectionAssert.Contains(foos, specialValue);
		}
	...
	[UsePluggable(typeof(WithImplSetByAttribute1))]
	[UsePluggable(typeof(WithImplSetByAttribute2), "some contract")]
